...
while True:                                              #循环开始
    ...
    while True:                                          
        ...
        if examine_ones(data):                           #如果通过奇偶检测
            ...
            data = data[1:].decode("unicode_escape")     #舍弃数据检验位并解码
            if data not in data_buf:                     #如果列表中不存在数据
                data_buf.append(data)                    #将数据添加到列表中
        else:                                            #反之
...
def examine_ones(data_b):                                                               #定义奇偶检测函数
    ...
    if nums_of_ones % 2 == 1 and examine_check.decode("unicode_escape") == "1":          #如果检验位为1并且消息位的1的个数是奇数
        return True                                                                     #返回True
    elif nums_of_ones % 2 == 0 and examine_check.decode("unicode_escape") == "0":        #如果检验位为0并且消息位的1的个数是偶数
        return True                                                                     #返回True
    else:                                                                               #其他情况均为错误
        return False                                                                    #返回False

def ack(data,addr,bool_situation_str_lower):                                            #定义发送ack函数（数据、地址、布尔情况
    data_tosend = data[1] + bool_situation_str_lower.encode("unicode_escape")           #拼接序号+布尔情况作为ack
